...
class rdma_dev {
  ...
  size_t alloc_conn_id() {
    std::lock_guard lg(conn_id_lock);
    return conn_ids.dequeue();
  }
  void free_conn_id(size_t conn_id) {
    std::lock_guard lg(conn_id_lock);
    conn_ids.enqueue(conn_id);
  }

  ibv_cq *create_cq(int cq_size) {
    return ibv_create_cq(ib_ctx, cq_size, nullptr, nullptr, 0);
  }

 public:
  rdma_dev(const char *dev_name = nullptr, int _ib_port = 1, int _gid_idx = 1);
  ~rdma_dev();
  ibv_mr *create_mr(size_t size, void *buf = nullptr,
                    int mr_flags = mr_flag_all);
   ...
   * @return ibv_mr* 创建的MR
   */
  ibv_mr *reg_mr(uint32_t mr_id, size_t size, void *buf = nullptr,
                 int mr_flags = mr_flag_all);
  // ibv_dm *create_dm(size_t size, uint32_t log_align = 3);
  // rdma_dmmr create_dmmr(size_t size, uint32_t log_align = 3,
  //                       int mr_flags = mr_flag_all);
  // rdma_dmmr reg_dmmr(uint32_t mr_id, size_t size, uint32_t log_align = 3,
  //                    int mr_flags = mr_flag_all);

#ifdef ENABLE_DOCA_DMA
  void enable_dma(const char *dev_name = nullptr);
  static void free_mmap_mem(void *addr, size_t len, void *opaque) {
    free_hugepage(addr, upper_align(len, 1 << 21));
  };
  std::tuple<doca_mmap *, void *> create_mmap(uint32_t mmap_id, size_t len,
                                              void *addr = nullptr);
  std::tuple<doca_mmap *, void *> reg_mmap(
      uint32_t mmap_id, std::tuple<doca_mmap *, void *> &mmpaddr);
  std::tuple<doca_mmap *, void *> reg_mmap(uint32_t mmap_id, size_t len,
                                           void *addr = nullptr);
#endif
...
class [[nodiscard]] rdma_future {
 ...
 public:
  ...
  rdma_future(rdma_future &&other)
      : cor(std::move(other.cor)), conn(std::move(other.conn)) {
    other.cor = nullptr;
    other.conn = nullptr;
  }

  constexpr bool await_ready() noexcept {
    return !cor || (cor->coro_state & coro_state_ready);
  }
...
class [[nodiscard]] rdma_buffer_future : public rdma_future {
 ...
 public:
  ...
  rdma_buffer_future(rdma_buffer_future &&other)
      : rdma_future(std::move(other)), _res_buf(other._res_buf) {
    other._res_buf = nullptr;
  }
  void *await_resume();
};
...
class [[nodiscard]] rdma_cas_future : public rdma_future {
 ...
 public:
  rdma_cas_future(rdma_coro *cor, rdma_conn *conn, void *res_buf,
                  uint64_t &cmpval)
      : rdma_future(cor, conn), _res_buf(res_buf), _cmpval(cmpval) {}
  rdma_cas_future(rdma_cas_future &&other)
      : rdma_future(std::move(other)),
        _res_buf(other._res_buf),
        _cmpval(other._cmpval) {
    other._res_buf = nullptr;
  }
  int await_resume();
};
...
class [[nodiscard]] rdma_cas_n_future : public rdma_future {
 ...
 public:
  rdma_cas_n_future(rdma_coro *cor, rdma_conn *conn, void *res_buf,
                    uint64_t &cmpval)
      : rdma_future(cor, conn), _res_buf(res_buf), _cmpval(cmpval) {}
...
class [[nodiscard]] dma_future {
 ...
 public:
  dma_future(rdma_worker *w, doca_buf *src, doca_buf *dst)
      : worker(w), src_buf(src), dst_buf(dst){};
  constexpr bool await_ready() noexcept { return false; }
  void await_suspend(std::coroutine_handle<> h) {
    worker->pending_dma_task->enqueue(std::move(h));
  };
  void await_resume();
};
...
class [[nodiscard]] dma_buf_future : public dma_future {
 ...
 public:
  dma_buf_future(rdma_worker *w, doca_buf *src, doca_buf *dst, void *buf)
      : dma_future(w, src, dst), res_buf(buf){};
  void *await_resume();
};
#endif
...
class rdma_worker : noncopyable {
 ...
 public:
  ...
  task_ring *pending_tasks{
      nullptr};  // WARNING: may overflow if too many conn arrive at same time
  handle_ring *yield_handler{nullptr};

  inline rdma_coro *alloc_coro(uint16_t conn_id);
  inline void free_coro(rdma_coro *cor);
  task<> cancel_coros(uint16_t conn_id, std::vector<int> &&cancel_list,
 ...
 public:
   ...
   */
  rdma_worker(rdma_dev &_dev, const ibv_qp_cap &_cap = so_qp_cap,
              int tempmp_size = rdma_default_tempmp_size,
              int max_coros = rdma_default_max_coros,
              int cq_size = rdma_default_cq_size);
  ~rdma_worker();
  inline void *alloc_buf(size_t size) { return mp->alloc(size); }
  template <is_integral... Ts>
  inline auto alloc_many(Ts... size) {
    return mp->alloc_many(size...);
  }
  ...
  auto run(TaskType &&input_task) {
    ...
    if (!value.has_value()) worker_loop();
  ...
  auto yield() {
    ...
    struct yield_awaiter {
      ...
      void await_suspend(std::coroutine_handle<> h) noexcept {
        worker->yield_handler->enqueue(std::move(h));
      }
   ...
   */
  rdma_conn *connect(const char *host = rdma_default_host,
                     int port = rdma_default_port);

#ifdef ENABLE_DOCA_DMA
  void enable_dma(uint32_t workq_size = dma_default_workq_size,
                  size_t buf_inv_size = dma_default_inv_buf_size);
#endif

  rdma_buffer_future read(uint64_t raddr, uint32_t len);
  rdma_future read(uint64_t raddr, void *laddr, uint32_t len, uint32_t lkey);
  rdma_future write(uint64_t raddr, void *laddr, uint32_t len,
                    uint32_t lkey = 0);
  void pure_write(uint64_t raddr, void *laddr, uint32_t len, uint32_t lkey = 0);
  rdma_cas_future fetch_add(uint64_t raddr, uint64_t &fetch, uint64_t addval);
  rdma_cas_future cas(uint64_t raddr, uint64_t &cmpval, uint64_t swapval);
  rdma_cas_n_future cas_n(uint64_t raddr, uint64_t cmpval, uint64_t swapval);
  rdma_faa_future faa(uint64_t raddr, uint64_t addval);
...
class rdma_server : public noncopyable {
 ...
 public:
   ...
   */
  void start_serve(std::function<task<>(rdma_conn *)> handler = nullptr,
                   int worker_num = 1, const ibv_qp_cap &qp_cap = zero_qp_cap,
                   int tempmp_size = rdma_default_tempmp_size,
                   int max_coros = rdma_default_max_coros,
                   int cq_size = rdma_default_cq_size,
                   int port = rdma_default_port);
...
class rdma_conn {
 public:
  ...
  std::unordered_map<uint32_t, std::variant<rdma_rmr, std::string>>
      exchange_idx;
 ...
 public:
  ...
  dma_future dma_read(doca_mmap *rmmp, uint64_t raddr, doca_mmap *lmmp,
                      void *laddr, size_t len);
  dma_future dma_write(doca_mmap *rmmp, uint64_t raddr, doca_mmap *lmmp,
                       void *laddr, size_t len);
  constexpr doca_mmap *lmmp() { return worker->mpmmp; }
#endif

  void pure_write(uint64_t raddr, uint32_t rkey, void *laddr, uint32_t len,
                  uint32_t lkey = 0);
  ...
  rdma_buffer_future read(const rdma_rmr &remote_mr, uint32_t offset,
                          uint32_t len) {
    return read(remote_mr.raddr + offset, remote_mr.rkey, len);
  }
  rdma_future read(uint64_t raddr, uint32_t rkey, void *laddr, uint32_t len,
                   uint32_t lkey);
  rdma_future read(const rdma_rmr &remote_mr, uint32_t offset, void *laddr,
                   uint32_t len, uint32_t lkey) {
    return read(remote_mr.raddr + offset, remote_mr.rkey, laddr, len, lkey);
  }
  rdma_future write(uint64_t raddr, uint32_t rkey, void *laddr, uint32_t len,
                    uint32_t lkey = 0);
  rdma_future write(const rdma_rmr &remote_mr, uint32_t offset, void *laddr,
                    uint32_t len, uint32_t lkey = 0) {
    return write(remote_mr.raddr + offset, remote_mr.rkey, laddr, len, lkey);
  }
  rdma_cas_future fetch_add(uint64_t raddr, uint32_t rkey, uint64_t &fetch,
                            uint64_t addval);
  rdma_cas_future cas(uint64_t raddr, uint32_t rkey, uint64_t &cmpval,
                      uint64_t swapval);
  rdma_cas_future cas(const rdma_rmr &remote_mr, uint32_t offset,
                      uint64_t &cmpval, uint64_t swapval) {
    return cas(remote_mr.raddr + offset, remote_mr.rkey, cmpval, swapval);
  }
  rdma_cas_n_future cas_n(uint64_t raddr, uint32_t rkey, uint64_t cmpval,
                          uint64_t swapval);
  rdma_cas_n_future cas_n(const rdma_rmr &remote_mr, uint32_t offset,
                          uint64_t cmpval, uint64_t swapval) {
  ...
  rdma_faa_future faa(const rdma_rmr &remote_mr, uint32_t offset,
                      uint64_t addval) {
  ...
  task<uint32_t> fill(uint64_t raddr, uint32_t rkey, uint32_t rlen,
                      void *fill_val, uint32_t fill_val_len,
                      uint32_t work_buf_size = 1024);
  task<uint32_t> fill(const rdma_rmr &remote_mr, uint32_t offset, uint32_t rlen,
                      void *fill_val, uint32_t fill_val_len,
                      uint32_t work_buf_size = 1024) {
    return fill(remote_mr.raddr + offset, remote_mr.rkey, rlen, fill_val,
                fill_val_len, work_buf_size);
  }

  inline void *alloc_buf(size_t size) { return worker->alloc_buf(size); }
  template <is_integral... Ts>
  inline auto alloc_many(Ts... size) {
    return worker->alloc_many(size...);
  }
...
inline void rdma_free_mr(ibv_mr *mr, const bool free_mem = true) {
  if (free_mem) free_hugepage(mr->addr, upper_align(mr->length, 1 << 21));
  if (ibv_dereg_mr(mr)) log_warn("failed dereg mr");
}
...
inline void rdma_free_wr(rdma_wr auto *wr, bool free_mem = true) {
  if (free_mem) free(wr->sg_list);
  free(wr);
}

// inline void rdma_free_dmmr(rdma_dmmr &&dmmr) {
//   auto [dm, mr] = dmmr;
//   if (mr && ibv_dereg_mr(mr)) log_warn("failed dereg mr");
//   if (dm && ibv_free_dm(dm)) log_warn("failed free dm");
// }
...
inline void fill_rw_wr(ibv_send_wr *wr, ibv_sge *sge, uint64_t raddr,
                       uint32_t rkey, void *laddr, uint32_t len,
                       uint32_t lkey) {
...
template <ibv_wr_opcode opcode>
inline void fill_atomic_wr(ibv_send_wr *wr, ibv_sge *sge, uint64_t raddr,
                           uint32_t rkey, void *laddr, uint32_t lkey,
                           uint64_t cmpadd, uint64_t swapval) {
  memset(wr, 0, sizeof(ibv_send_wr));
  sge->addr = (uint64_t)laddr;
...
inline void fill_recv_wr(ibv_recv_wr *wr, ibv_sge *sge, void *laddr,
                         uint32_t len, uint32_t lkey) {

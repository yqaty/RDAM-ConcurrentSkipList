...
struct Config {
  ...
  void ParseArg(int argc, char *argv[]) {
    ...
    cmd_parser.add<std::string>("server_ip", 'b', "IP address of server",
                                false);
    cmd_parser.add<uint64_t>("num_cli", 'c', "Number of client", false, 4,
                             cmdline::range(1, 80));
    cmd_parser.add<uint64_t>("num_machine", 'd', "Number of client", false, 4,
                             cmdline::range(1, 80));
    cmd_parser.add("roce", 'e', "Type of unerlying internet");
    cmd_parser.add<uint64_t>(
        "max_coro", 'f', "Number of max coroutine in each thread", false, 256);
    ...
    cmd_parser.add<uint64_t>("num_op", 'j',
                             ...
                             1000000);
    cmd_parser.add<uint64_t>("num_coro", 'k', "Number of coro", false, 4,
                             cmdline::range(1, 80));
    cmd_parser.add<uint64_t>("machine_id", 'l', "machine_id", false, 0,
                             cmdline::range(0, 10));
    ...
    roce_flag = cmd_parser.exist("roce");

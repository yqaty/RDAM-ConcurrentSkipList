...
Config config;
uint64_t load_num;
using ClientType = SKIPLIST::Client;
using ServerType = SKIPLIST::Server;

inline uint64_t GenKey(uint64_t key) {
#ifdef ORDERED_INSERT
  return key;
#else
  return MySkipList::rand_();
#endif
}
...
task<> load(Client *cli, uint64_t cli_id, uint64_t coro_id) {
  ...
  uint64_t key, value;
  uint64_t num_op =
      load_num / (config.num_machine * config.num_cli * config.num_coro);
  for (uint64_t i = 0; i < num_op; i++) {
    key = GenKey((config.machine_id * config.num_cli * config.num_coro +
                  cli_id * config.num_coro + coro_id) *
                     num_op +
                 i);
    value = GenKey((config.machine_id * config.num_cli * config.num_coro +
                    cli_id * config.num_coro + coro_id) *
                       num_op +
                   i);
...
//   double update_frac =
//       config.insert_frac + config.read_frac + config.update_frac;
//   xoshiro256pp op_chooser;
//   xoshiro256pp key_chooser;
//   uint64_t num_op =
//       config.num_op / (config.num_machine * config.num_cli *
//       config.num_coro);
//   uint64_t load_avr =
//       load_num / (config.num_machine * config.num_cli * config.num_coro);
...
//       tmp_key = GenKey(load_num +
//                        (config.machine_id * config.num_cli * config.num_coro
//                        +
//                         cli_id * config.num_coro + coro_id) *
//                            load_avr +
...
//       ret_value.len = 0;
//       tmp_key = GenKey((config.machine_id * config.num_cli * config.num_coro
//       +
//                         cli_id * config.num_coro + coro_id) *
//                            load_avr +
//                        gen->operator()(key_chooser()));
...
//       if (ret_value.len != value.len ||
//           memcmp(ret_value.data, value.data, value.len) != 0) {
//         // log_err("[%lu:%lu]wrong value for key:%lu with value:%s
//         expected:%s
//         // ret_value.len:%lu value.len:%lu", cli_id, coro_id, tmp_key,
...
//       // if (slot_ptr == 0ull)
//       //     log_err("[%lu:%lu]update for key:%lu result in loss", cli_id,
//       //     coro_id, tmp_key);
//       // else if (ret_value.len != update_value.len || memcmp(ret_value.data,
//       // update_value.data, update_value.len) != 0)
//       // {
//       //     log_err("[%lu:%lu]wrong value for key:%lu with value:%s
//       //     expected:%s", cli_id, coro_id, tmp_key,
...
//       //     auto [slot_ptr, slot] = co_await cli->search(&key, &ret_value);
//       //     if (slot_ptr != 0ull)
//       //         log_err("[%lu:%lu]fail to delete value for key:%lu with
//       //         slot_ptr:%lx and ret_value:%s", cli_id, coro_id,
//       //         tmp_key,slot_ptr,ret_value.data);
//       //     else
//       //         break;
//       //     if(cnt++>=3){
//       //         exit(-1);
  ...
  if (config.is_server) {
    ...
    for (uint64_t i = 0; i < config.num_cli; i++) {
      ...
      for (uint64_t j = 0; j < config.num_coro; j++) {
        lmrs[i * config.num_coro + j] = dev.create_mr(
            cbuf_size, mem_buf + cbuf_size * (i * config.num_coro + j));
        ClientType *cli;
        cli = new ClientType(config, lmrs[i * config.num_coro + j],
                             rdma_clis[i], rdma_conns[i], rdma_wowait_conns[i],
    ...
    //                    typeid(ClientType) ==
    //                    typeid(ClevelSingleFilter::Client);
    // ;

    // if (config.machine_id == 0 && rehash_flag) {
    //   // rdma_clis[config.num_cli] = new rdma_client(dev, so_qp_cap,
    //   // rdma_default_tempmp_size, config.max_coro, config.cq_size);
    //   // rdma_conns[config.num_cli] =
    //   // rdma_clis[config.num_cli]->connect(config.server_ip);
    //   // rdma_wowait_conns[config.num_cli] =
    //   // rdma_clis[config.num_cli]->connect(config.server_ip);

    //   // lmrs[config.num_cli * config.num_coro] =
    //   //         dev.create_mr(cbuf_size, mem_buf + cbuf_size *
    //   (config.num_cli
    //   //         * config.num_coro));
    //   // ClientType * rehash_cli = new ClientType(config, lmrs[config.num_cli
    //   *
    //   // config.num_coro], rdma_clis[config.num_cli],
    //   //
    //   rdma_conns[config.num_cli],rdma_wowait_conns[config.num_cli],config.machine_id,
    //   // config.num_cli, config.num_coro); auto th = [&](rdma_client
    //   *rdma_cli)
    //   // {
    ...
    //             gens[cli_id * config.num_coro + j],
    //             (ClientType *)(clis[cli_id * config.num_coro + j]), cli_id,
    //             j));
    //       }
    //       rdma_cli->run(gather(std::move(tasks)));
    //     };
    ...
    //   duration = std::chrono::duration<double, std::milli>(end -
    //   start).count(); printf("Run duration:%.2lfms\n", duration); printf("Run
    //   IOPS:%.2lfKops\n", op_cnt / duration); fflush(stdout);
    ...
    //     (config.num_cli
    //     //         * config.num_coro));
    //     // ClientType* check_cli = new ClientType(config, lmrs[config.num_cli
    //     *
    //     // config.num_coro], rdma_clis[config.num_cli],
    //     //
    //     rdma_conns[config.num_cli],rdma_wowait_conns[config.num_cli],config.machine_id,
    //     // config.num_cli, config.num_coro);

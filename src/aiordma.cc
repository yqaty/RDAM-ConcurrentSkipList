...
struct sock_select_list {
  ...
  void del(int sk) {
    ...
    *psk = -1;
    if (maxfd == sk) maxfd = *std::ranges::max_element(sks);
    dirty = true;
  }
  void cleanup() {
    size_t i = 0, j = 0;
    for (; j < sks.size(); ++i, ++j) {
      while (j < sks.size() && sks[j] == -1) ++j;
      sks[i] = sks[j];
    }
    for (; i < j; ++i) sks.pop_back();
    dirty = false;
  }
  int select() {
    if (dirty) cleanup();
    ...
    FD_ZERO(&fds);
    std::ranges::for_each(sks, [this](int sk) { FD_SET(sk, &fds); });
...
rdma_dev::rdma_dev(const char *dev_name, int _ib_port, int _gid_idx)
  ...
  for (int i = 1; i < 65536; ++i) conn_ids.enqueue(i);

  log_info("rdma_dev init success");
}

rdma_dev::~rdma_dev() {
  if (info_mr) rdma_free_mr(info_mr, true);
  if (pd && ibv_dealloc_pd(pd)) log_warn("failed to dealloc pd");
  if (ib_ctx && ibv_close_device(ib_ctx)) log_warn("failed to close device");
#ifdef ENABLE_DOCA_DMA
...
ibv_mr *rdma_dev::create_mr(size_t size, void *buf, int mr_flags) {
  ...
  if (!mr && malloc_flag) free_hugepage(buf, upper_align(size, 1 << 21));
  return mr;
}
...
ibv_mr *rdma_dev::reg_mr(uint32_t mr_id, ibv_mr *mr) {
  ...
  auto hdr = (rdma_infomr_hdr *)info_mr->addr;
  if (hdr->tail + sizeof(rdma_rmr) > rdma_info_mr_size ||
      info_idx.contains(mr_id))
    return nullptr;
...
void rdma_dev::enable_dma(const char *dev_name) {
  ...
  if (!dev_name)
    ...
    for (uint32_t i = 0; !devinfo && i < devcount; ++i)
      for (auto [property, len] :
           std::initializer_list<std::tuple<doca_devinfo_property, uint32_t>>{
        ...
        if (doca_devinfo_property_get(devlist[i], property, property_buf,
                                      len) == DOCA_SUCCESS &&
            memcmp(dev_name_raw, property_buf, dev_name_len) == 0) {
          devinfo = devlist[i];
          break;
        }
  }
...
std::tuple<doca_mmap *, void *> rdma_dev::create_mmap(uint32_t mmap_id,
                                                      size_t len, void *addr) {
  bool alloc_flag = false;
  doca_mmap *mmp = nullptr;
  assert_require(doca_mmap_create(std::to_string(mmap_id).c_str(), &mmp) ==
                 DOCA_SUCCESS);
  assert_require(doca_mmap_start(mmp) == DOCA_SUCCESS);
  assert_require(doca_mmap_dev_add(mmp, dma_dev) == DOCA_SUCCESS);
  assert_require(addr != nullptr ||
                 (alloc_flag = (addr = alloc_hugepage(len)) != nullptr));
  assert_require(doca_mmap_populate(mmp, (char *)addr, len, 1 << 21,
                                    alloc_flag ? free_mmap_mem : nullptr,
                                    nullptr) == DOCA_SUCCESS);
  return {mmp, addr};
}

std::tuple<doca_mmap *, void *> rdma_dev::reg_mmap(
    uint32_t mmap_id, std::tuple<doca_mmap *, void *> &mmpaddr) {
  void *export_desc;
  size_t export_desc_len;
  auto [mmp, addr] = mmpaddr;
  if (!mmp) return {nullptr, nullptr};
  assert_require(doca_mmap_export(mmp, dma_dev, &export_desc,
                                  &export_desc_len) == DOCA_SUCCESS);
  std::lock_guard _(info_lock);
  auto hdr = (rdma_infomr_hdr *)info_mr->addr;
  if (hdr->tail + sizeof(rdma_rmmap) + export_desc_len > rdma_info_mr_size ||
      info_idx.contains(mmap_id)) {
    free(export_desc);
...
std::tuple<doca_mmap *, void *> rdma_dev::reg_mmap(uint32_t mmap_id, size_t len,
                                                   void *addr) {
...
rdma_worker::rdma_worker(rdma_dev &_dev, const ibv_qp_cap &_qp_cap,
  ...
  if (tempmp_size > 0) {
    ...
    assert_require(mpmr =
                       dev.create_mr(mp->get_data_len(), mp->get_data_addr()));
  }
  if (max_coros > 8) {
  ...
  if (qp_cap.max_recv_wr > 0) assert_require(pending_tasks = new task_ring());
  assert_require(yield_handler = new handle_ring(max_coros));
}

rdma_worker::~rdma_worker() {
  if (cq && ibv_destroy_cq(cq)) log_warn("failed to destory cq");
  if (mpmr) rdma_free_mr(mpmr, false);
  if (mp) delete mp;
  if (coros) delete[] coros;
  if (pending_tasks) delete pending_tasks;
  if (yield_handler) delete yield_handler;
#ifdef ENABLE_DOCA_DMA
  ...
  if (pending_dma_task) delete pending_dma_task;
#endif
}

rdma_coro *rdma_worker::alloc_coro(uint16_t conn_id) {
  if (free_head == rdma_coro_none) return nullptr;
  auto local_head = free_head;
  rdma_coro *res = coros + local_head;
#ifdef ALLOC_CORO_THREAD_SAFE
  while (!__atomic_compare_exchange_n(&free_head, &local_head, res->next, false,
...
void rdma_worker::free_coro(rdma_coro *cor) {
  ...
  cor->next = free_head;
#ifdef ALLOC_CORO_THREAD_SAFE
  while (!__atomic_compare_exchange_n(&free_head, &cor->next, cor->id, false,
                                      __ATOMIC_ACQUIRE, __ATOMIC_ACQUIRE))
    ;
#else
  free_head = cor->id;
...
task<> rdma_worker::cancel_coros(uint16_t conn_id,
  ...
  for (auto e : cancel_list)
    if (auto &&cor = coros[e]; cor.ctx == conn_id) {
      ...
      if (cor.coro_state & coro_state_inited) cor.resume_handler();
    }
  finish_flag = 1;
  co_return;
}
...
#endif
  while (loop_flag) {
    assert_require((ready_num = ibv_poll_cq(cq, rdma_max_wc_per_poll, wcs)) >=
                   0);
    for (int i = 0; i < ready_num; ++i) {
      ...
      if (wc.status != IBV_WC_SUCCESS) {
        log_warn("got bad completion with status: 0x%x, vendor syndrome: 0x%x",
                 wc.status, wc.vendor_err);
    ...
    if (yield_handler && yield_handler->count()) yield_handler->dequeue()();
#ifdef ENABLE_DOCA_DMA
    if (dma_workq && (dma_workq_res = doca_workq_progress_retrieve(
                          dma_workq, &event, DOCA_WORKQ_RETRIEVE_FLAGS_NONE)) !=
                         DOCA_ERROR_AGAIN) {
      assert_require(dma_workq_res == DOCA_SUCCESS &&
                     event.result.u64 ==
                         DOCA_SUCCESS);  // TODO: state as future result
...
void rdma_worker::enable_dma(uint32_t workq_size, size_t buf_inv_size) {
  assert_require(buf_inv == nullptr && dma == nullptr && dma_ctx == nullptr &&
                 dma_workq == nullptr);
  assert_require(doca_buf_inventory_create("buf_inventory", buf_inv_size,
                                           DOCA_BUF_EXTENSION_NONE,
                                           &buf_inv) == DOCA_SUCCESS);
  ...
  if (mp) {
    std::tie(mpmmp, std::ignore) = dev.create_mmap(
        dma_tempmp_mmap_name, mp->get_data_len(), mp->get_data_addr());
...
rdma_buffer_future rdma_worker::read(uint64_t raddr, uint32_t len) {
  auto rmi = apmap[raddr >> 56];
  return rmi.conn->read(raddr & mask(48), rmi.rmr.rkey, len);
}
rdma_future rdma_worker::read(uint64_t raddr, void *laddr, uint32_t len,
                              uint32_t lkey) {
  auto rmi = apmap[raddr >> 56];
  return rmi.conn->read(raddr & mask(48), rmi.rmr.rkey, laddr, len, lkey);
}
rdma_future rdma_worker::write(uint64_t raddr, void *laddr, uint32_t len,
                               uint32_t lkey) {
  auto rmi = apmap[raddr >> 56];
  return rmi.conn->write(raddr & mask(48), rmi.rmr.rkey, laddr, len, lkey);
}

void rdma_worker::pure_write(uint64_t raddr, void *laddr, uint32_t len,
                             uint32_t lkey) {
  auto rmi = apmap[raddr >> 56];
  return rmi.conn->pure_write(raddr & mask(48), rmi.rmr.rkey, laddr, len, lkey);
}

rdma_cas_future rdma_worker::fetch_add(uint64_t raddr, uint64_t &fetch,
                                       uint64_t addval) {
  auto rmi = apmap[raddr >> 56];
  return rmi.conn->fetch_add(raddr & mask(48), rmi.rmr.rkey, fetch, addval);
}
rdma_cas_future rdma_worker::cas(uint64_t raddr, uint64_t &cmpval,
                                 uint64_t swapval) {
  auto rmi = apmap[raddr >> 56];
  return rmi.conn->cas(raddr & mask(48), rmi.rmr.rkey, cmpval, swapval);
}
rdma_cas_n_future rdma_worker::cas_n(uint64_t raddr, uint64_t cmpval,
                                     uint64_t swapval) {
  auto rmi = apmap[raddr >> 56];
  return rmi.conn->cas_n(raddr & mask(48), rmi.rmr.rkey, cmpval, swapval);
}
rdma_faa_future rdma_worker::faa(uint64_t raddr, uint64_t addval) {
  auto rmi = apmap[raddr >> 56];
  return rmi.conn->faa(raddr & mask(48), rmi.rmr.rkey, addval);
}

rdma_server::~rdma_server() { stop_serve(); }

void rdma_server::start_serve(std::function<task<>(rdma_conn *)> handler,
                              int worker_num, const ibv_qp_cap &qp_cap,
                              int tempmp_size, int max_coros, int cq_size,
                              int port) {
  ...
  {
    int optval = 1;
    assert_require(0 == setsockopt(listenfd, SOL_SOCKET, SO_REUSEPORT, &optval,
                                   sizeof(optval)));

    int sflag;
    assert_require((sflag = fcntl(listenfd, F_GETFL, 0)) != -1 &&
                   fcntl(listenfd, F_SETFL, sflag | O_NONBLOCK) != -1);
  }
  ...
  assert_require(::bind(listenfd, (sockaddr *)&local_addr, sizeof(sockaddr)) !=
                 -1);
  ...
  if (handler) {
    ...
    for (int i = 0; i < worker_num; ++i) {
      assert_require(workers.emplace_back(new rdma_worker(
          dev,
          qp_cap.max_send_wr == 0 && qp_cap.max_recv_wr == 0 ? sr_qp_cap
                                                             : qp_cap,
          tempmp_size, max_coros, cq_size)));
      worker_threads.emplace_back(
          std::thread(&rdma_worker::worker_loop, workers[i]));
    }
  } else {
    assert_require(worker_num == 1);
    ...
    while (!st.stop_requested()) {
      if (select_list.select()) {
        ...
        for (size_t i = 0; i < sk_num; ++i) {
          int sk = sks[i];
          if (!select_list.isset(sk)) continue;
          if (sk == listenfd) {
            int accepted_sock =
                ::accept(listenfd, (sockaddr *)&remote_addr, &addr_len);
            assert_require(accepted_sock > 0);
            select_list.add(accepted_sock);
            // log_err("accept conn: %d", accepted_sock);
            ...
            if (recv_len <= 0) {
              ...
              switch (((__rdma_exchange_t *)recv_buf)->proto) {
                ...
                case rdma_exchange_proto_ready:
                  if (handler)
                    workers[sk % worker_num]->pending_tasks->enqueue(
                        handler(conn));
                  break;
...
rdma_conn *rdma_worker::connect(const char *host, int port) {
  ...
  if (!conn) log_err("failed to alloc conn");
  size_t recv_len = 0;
  while ((recv_len += recv(sock, recv_buf + recv_len,
                           rdma_sock_recv_buf_size - recv_len, 0)) <
         sizeof(__rdma_exchange_t))
    ;
  log_info("RTR -> RTS");
  conn->handle_recv_setup(recv_buf, recv_len);
  recv_len = 0;
...
rdma_conn::rdma_conn(rdma_worker *w, int _sock)
  ...
  worker_conn_id = worker->conn_cnt++;
  // assert_require(worker_conn_id < 256);
  {
  ...
  assert_require(exchange_mr =
                     dev.create_mr(rdma_info_mr_size, nullptr, mr_flag_lo));
...
rdma_conn::~rdma_conn() {
  ...
  if (qp && ibv_destroy_qp(qp)) log_warn("failed to destroy qp");
  if (exchange_mr) rdma_free_mr(exchange_mr);
...
void rdma_conn::release_working_coros() {
  if (!worker || !worker->coros || !worker->pending_tasks) return;
  std::vector<int> cancel_list;
  for (int i = 0; i < worker->max_coros; ++i)
    if (worker->coros[i].ctx == conn_id) cancel_list.push_back(i);
  volatile int finish_flag = 0;
  if (worker->loop_flag)  // WARNING: should not close conn during stopping work
                          // loop
    worker->pending_tasks->enqueue(
        worker->cancel_coros(conn_id, std::move(cancel_list), finish_flag));
  else
    worker->run(
        worker->cancel_coros(conn_id, std::move(cancel_list), finish_flag));
  while (finish_flag == 0)
...
void rdma_conn::send_exchange(uint16_t proto) {
  ...
  if (proto == rdma_exchange_proto_setup) {
    if (dev.gid_idx >= 0) {
      assert_require(
          0 == ibv_query_gid(dev.ib_ctx, dev.ib_port, dev.gid_idx, &local_gid));
    ...
    assert_require(::send(sock, &exc, sizeof(__rdma_exchange_t), 0) ==
                   sizeof(__rdma_exchange_t));
...
int rdma_conn::modify_qp_to_rtr(uint32_t rqp_num, uint16_t rlid,
                                const ibv_gid *rgid) {
...
task<> rdma_conn::update_cache() {
  auto hdr = (rdma_infomr_hdr *)exchange_mr->addr;
  exchange_wr->sg_list->length =
      std::min((uint64_t)rdma_info_mr_size - sizeof(rdma_infomr_hdr),
               hdr->tail + rdma_info_mr_probing);
  assert_check(co_await do_send(exchange_wr, exchange_wr));
  while (crc64(hdr->data, std::min(hdr->tail, (uint64_t)rdma_info_mr_size -
                                                  sizeof(rdma_infomr_hdr))) !=
         hdr->cksum) {
    exchange_wr->sg_list->length =
        std::min((uint64_t)rdma_info_mr_size - sizeof(rdma_infomr_hdr),
                 hdr->tail + rdma_info_mr_probing);
  ...
  for (uint64_t i = 0; i < hdr->tail;) {
    ...
    if (info->type == rdma_info_type_mr) {
      auto tmp = *(rdma_rmr *)(hdr->data + i);
      tmp.raddr |= (uint64_t)worker_conn_id << 56;
      exchange_idx.emplace((uint32_t)info->id, tmp);
      i += sizeof(rdma_rmr);
      worker->apmap[worker_conn_id] = ap2rmi_t{this, tmp};
    } else if (info->type == rdma_info_type_mmap) {
      exchange_idx.emplace((uint32_t)info->id,
                           std::string((char *)info->data, info->len));
...
task<rdma_rmr> rdma_conn::query_remote_mr(uint32_t mr_id) {
  if (!exchange_idx.contains(mr_id)) co_await update_cache();
  if (exchange_idx.contains(mr_id))
    if (auto res = std::get_if<rdma_rmr>(&exchange_idx[mr_id])) co_return *res;
  log_err("mr not found");
  co_return rdma_rmr{};
}

#ifdef ENABLE_DOCA_DMA
task<std::tuple<doca_mmap *, uint64_t>> rdma_conn::query_remote_mmap(
    uint32_t mmap_id) {
  doca_mmap *mmp = nullptr;
  uint64_t addr = 0;
  if (!exchange_idx.contains(mmap_id)) co_await update_cache();
  if (exchange_idx.contains(mmap_id))
    if (auto res = std::get_if<std::string>(&exchange_idx[mmap_id])) {
      assert_check(doca_mmap_create_from_export(
                       std::to_string(mmap_id).c_str(), (uint8_t *)res->c_str(),
                       res->length(), worker->dev.dma_dev,
                       &mmp) == DOCA_SUCCESS);
...
inline void fill_dma_job(doca_dma_job_memcpy *dma_job, doca_ctx *dma_ctx,
                         doca_buf *src_buf, doca_buf *dst_buf, size_t len) {
...
dma_buf_future rdma_conn::dma_read(doca_mmap *rmmp, uint64_t raddr,
                                   size_t len) {
  auto [buf, job] = alloc_many(len, sizeof(doca_dma_job_memcpy));
  assert_check(buf);
  doca_buf *src_buf, *dst_buf;
  assert_check(doca_buf_inventory_buf_by_addr(worker->buf_inv, rmmp,
                                              (char *)raddr, len,
                                              &src_buf) == DOCA_SUCCESS);
  assert_check(doca_buf_inventory_buf_by_addr(worker->buf_inv, worker->mpmmp,
                                              (char *)buf, len,
                                              &dst_buf) == DOCA_SUCCESS);
  auto dma_job = (doca_dma_job_memcpy *)job;
  fill_dma_job(dma_job, worker->dma_ctx, src_buf, dst_buf, len);
  assert_check(doca_workq_submit(worker->dma_workq, &dma_job->base) ==
               DOCA_SUCCESS);
  return dma_buf_future(worker, src_buf, dst_buf, buf);
}

dma_future rdma_conn::dma_read(doca_mmap *rmmp, uint64_t raddr, doca_mmap *lmmp,
                               void *laddr, size_t len) {
  ...
  assert_check(doca_buf_inventory_buf_by_addr(worker->buf_inv, rmmp,
                                              (char *)raddr, len,
                                              &src_buf) == DOCA_SUCCESS);
  assert_check(doca_buf_inventory_buf_by_addr(worker->buf_inv, lmmp,
                                              (char *)laddr, len,
                                              &dst_buf) == DOCA_SUCCESS);
  fill_dma_job(dma_job, worker->dma_ctx, src_buf, dst_buf, len);
  assert_check(doca_workq_submit(worker->dma_workq, &dma_job->base) ==
...
dma_future rdma_conn::dma_write(doca_mmap *rmmp, uint64_t raddr,
                                doca_mmap *lmmp, void *laddr, size_t len) {
  auto dma_job = (doca_dma_job_memcpy *)alloc_buf(sizeof(doca_dma_job_memcpy));
  assert_check(dma_job);
  doca_buf *src_buf, *dst_buf;
  assert_check(doca_buf_inventory_buf_by_addr(worker->buf_inv, rmmp,
...
int rdma_future::await_resume() {
  if (conn && cor) conn->worker->free_coro(cor);
  return !(cor->coro_state &
           coro_state_error);  // single thread thus cor is vaild after free
}
...
int rdma_cas_future::await_resume() {
  ...
  if (!rdma_future::await_resume()) return false;
...
uint64_t rdma_faa_future::await_resume() {
  conn->free_buf(_res_buf);
  if (!rdma_future::await_resume()) throw "faa failed";
  return *(uint64_t *)_res_buf;
}

rdma_buffer_future rdma_conn::read(uint64_t raddr, uint32_t rkey,
                                   uint32_t len) {
  auto [buf, sge, wr] = alloc_many(len, sizeof(ibv_sge), sizeof(ibv_send_wr));
  assert_check(buf);
  auto send_wr = (ibv_send_wr *)wr;
  fill_rw_wr<IBV_WR_RDMA_READ>(send_wr, (ibv_sge *)sge, raddr, rkey, buf, len,
                               lkey());
  auto fur = do_send(send_wr, send_wr);
  return rdma_buffer_future(fur.cor, fur.conn, buf);
}

rdma_future rdma_conn::read(uint64_t raddr, uint32_t rkey, void *laddr,
                            uint32_t len, uint32_t lkey) {
  auto [sge, wr] = alloc_many(sizeof(ibv_sge), sizeof(ibv_send_wr));
  assert_check(sge);
  auto send_wr = (ibv_send_wr *)wr;
  fill_rw_wr<IBV_WR_RDMA_READ>(send_wr, (ibv_sge *)sge, raddr, rkey, laddr, len,
                               lkey);
...
rdma_future rdma_conn::write(uint64_t raddr, uint32_t rkey, void *laddr,
                             uint32_t len, uint32_t _lkey) {
  auto [sge, wr] = alloc_many(sizeof(ibv_sge), sizeof(ibv_send_wr));
  assert_check(sge);
  auto send_wr = (ibv_send_wr *)wr;
  fill_rw_wr<IBV_WR_RDMA_WRITE>(send_wr, (ibv_sge *)sge, raddr, rkey, laddr,
                                len, _lkey);
  if (_lkey == 0) send_wr->send_flags = IBV_SEND_INLINE;
  auto res = do_send(send_wr, send_wr);
  free_buf(sge);

  // For Flush
  //  auto [sge,wr,rr] = alloc_many(sizeof(ibv_sge),
  //  sizeof(ibv_send_wr),sizeof(ibv_send_wr)); assert_check(sge); auto send_wr
  //  = (ibv_send_wr *)wr; auto read_wr = (ibv_send_wr *)rr;
  //  fill_rw_wr<IBV_WR_RDMA_WRITE>(send_wr, (ibv_sge *)sge, raddr, rkey, laddr,
  //  len, _lkey); fill_rw_wr<IBV_WR_RDMA_READ>(read_wr, (ibv_sge *)sge, raddr,
  //  rkey, laddr, len, _lkey); send_wr->next = read_wr; if (_lkey == 0)
...
void rdma_conn::pure_write(uint64_t raddr, uint32_t rkey, void *laddr,
                           uint32_t len, uint32_t _lkey) {
  auto [sge, wr] = alloc_many(sizeof(ibv_sge), sizeof(ibv_send_wr));
  assert_check(sge);
  auto send_wr = (ibv_send_wr *)wr;
...
// rdma_cas_future rdma_conn::fetch_add(uint64_t raddr, uint32_t rkey, uint64_t
// &cmpval, uint64_t swapval)
rdma_cas_future rdma_conn::fetch_add(uint64_t raddr, uint32_t rkey,
                                     uint64_t &fetch, uint64_t addval) {
  auto [buf, sge, wr] =
      alloc_many(sizeof(uint64_t), sizeof(ibv_sge), sizeof(ibv_send_wr));
  assert_check(buf);
  auto send_wr = (ibv_send_wr *)wr;
  fill_atomic_wr<IBV_WR_ATOMIC_FETCH_AND_ADD>(send_wr, (ibv_sge *)sge, raddr,
                                              rkey, buf, lkey(), addval, 0);
  auto fur = do_send(send_wr, send_wr);
  return rdma_cas_future(fur.cor, fur.conn, buf, fetch);
}

rdma_cas_future rdma_conn::cas(uint64_t raddr, uint32_t rkey, uint64_t &cmpval,
                               uint64_t swapval) {
  auto [buf, sge, wr] =
      alloc_many(sizeof(uint64_t), sizeof(ibv_sge), sizeof(ibv_send_wr));
  assert_check(buf);
  auto send_wr = (ibv_send_wr *)wr;
  fill_atomic_wr<IBV_WR_ATOMIC_CMP_AND_SWP>(send_wr, (ibv_sge *)sge, raddr,
                                            rkey, buf, lkey(), cmpval, swapval);
  auto fur = do_send(send_wr, send_wr);
  return rdma_cas_future(fur.cor, fur.conn, buf, cmpval);
}

rdma_cas_n_future rdma_conn::cas_n(uint64_t raddr, uint32_t rkey,
                                   uint64_t cmpval, uint64_t swapval) {
  auto [buf, sge, wr] =
      alloc_many(sizeof(uint64_t), sizeof(ibv_sge), sizeof(ibv_send_wr));
  assert_check(buf);
  auto send_wr = (ibv_send_wr *)wr;
...
task<uint32_t> rdma_conn::fill(uint64_t raddr, uint32_t rkey, uint32_t rlen,
                               void *fill_val, uint32_t fill_val_len,
                               uint32_t work_buf_size) {
  assert_check(is_times_ofN(work_buf_size, fill_val_len));
  auto [buf, sge, wr] =
      alloc_many(work_buf_size, sizeof(ibv_sge), sizeof(ibv_send_wr));
  assert_check(buf);
  uint32_t filled = 0;
  ...
  auto send_wr = (ibv_send_wr *)wr;
  fill_rw_wr<IBV_WR_RDMA_WRITE>(send_wr, (ibv_sge *)sge, raddr, rkey, buf,
                                work_buf_size, lkey());
  filled = 0;
  for (; filled + work_buf_size <= rlen;) {
    if (!co_await do_send(send_wr, send_wr)) co_return filled;
...
std::tuple<uint64_t, uint64_t> get_addrlen_from_export(
  ...
  auto res =
      std::make_tuple(json_object_get_int64(addr), json_object_get_int64(jlen));
